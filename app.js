...
const isLyricsPage = location.pathname.endsWith("lyrics.html") || location.pathname.endsWith("lyrics-live.html") || location.pathname.endsWith("lyrics-live-v2.html");
...
if (!isLyricsPage) {
  ...
  if (params.get("code")) exchangeCode(params.get("code")).then(() => { location.replace(`lyrics-live-v2.html?v=${APP_VERSION}`); }).catch(error => { setConnection("LOGIN ERROR", `Spotify отклонил вход: ${error.message}`); console.error(error); });
} else {
